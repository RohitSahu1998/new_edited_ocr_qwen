        ...
        with st.spinner("Extracting with PaddleOCR and Qwen. This could take a while..."):
            try:
                ...
                if not matched_data:
                    ...
                    # Since CV2 imread doesn't work on PDFs, we convert all pages into distinct images
                    drawing_image_paths = []
                    if temp_path.lower().endswith('.pdf'):
                        pages = convert_from_path(temp_path)
                        for i, page in enumerate(pages):
                            p_path = f"temp_drawing_target_page_{i+1}.jpg"
                            page.convert('RGB').save(p_path)
                            drawing_image_paths.append(p_path)
                    else:
                        drawing_image_paths = [temp_path]

                    # 5. Highlight and Export Data
                    base_highlight_jpg = "output_highlighted.jpg"
                    highlight_pdf_path = "output_highlighted.pdf"
                    excel_path = "extracted_data.xlsx"
                    
                    # Generate JPG(s) for the Streamlit UI Preview
                    highlight_matches_on_image(drawing_image_paths, matched_data, base_highlight_jpg)
                    
                    # Generate a single multi-page PDF specifically for the user to download
                    highlight_matches_on_image(drawing_image_paths, matched_data, highlight_pdf_path)
                    
                    rendered_jpgs = [base_highlight_jpg]
                    if len(drawing_image_paths) > 1:
                        for i in range(1, len(drawing_image_paths)):
                            rendered_jpgs.append(f"output_highlighted_page_{i+1}.jpg")
                    ...
                    with col1:
                        ...
                        st.image(rendered_jpgs, use_container_width=True)
                        
                        # Provide Download Button for Highlighted PDF
                        with open(highlight_pdf_path, "rb") as file:
                            st.download_button(
                                label="✅ Download Highlighted Multi-Page Document (.pdf)",
                                data=file,
                                file_name=f"highlighted_{uploaded_file.name}.pdf" if uploaded_file.name.endswith('.pdf') else "highlighted_output.pdf",
